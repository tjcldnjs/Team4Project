...
public class Query {
	...
	String reservation = " SELECT ri.name as 예약자, cm.carname as 차량명, ci.cartype as 차종, ci.brand as 차량브랜드, "
			...
			+ "from reservation_info as ri " + "join reservation_person_info as rp on ri.name = rp.name "
			+ "join recruittable as re on re.id = ri.id " + "join carmanagement as cm on cm.carid = re.carid "
			+ "join carinfo as ci on ci.carname = cm.carname " + "where ri.id = ? ";
	...
	String SELEC_PUEL = " select cm.carname as 차량명, ci.cartype as 차종, ci.brand as 차량브랜드, "
			+ "ci.puel as 유종, ci.needlicence as 요구면허, ci.priceperday as 1일당가격 " + "from carmanagement as cm "
			+ "join carinfo as ci on cm.carname = ci.carname" + "where ci.puel = ? ";
